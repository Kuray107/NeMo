...
def get_sampler(config, model):
    type = config.get("type", "ancestral_cache")    
    if type == 'ancestral_cache':
        return AncestralCacheSampler(config, model)
    elif type == 'ancestral-conf-top-k':
        return AncestralConfTopKSampler(config, model)
    elif type == 'dimple':
        return DimpleSampler(config, model)
    # elif type == 'remdm-cap':
...
class Sampler:
    def __init__(self, config, model):
        ...
        self.num_steps = config.get("num_steps", 200)
...
class AncestralConfTopKSampler(Sampler):
    ...
    def update(self, current_ids, copy_flag, alpha_t, alpha_s, log_probs, is_last_step=False, **kwargs):
        ...
        if is_last_step:
            num_tokens_per_sample = masked_flag.sum(dim=1)
        else:
...
class DimpleSampler(Sampler):
    def __init__(self, config, model):
        ...
        logging.info(f"DimpleSampler initialized with num_steps: {self.num_steps}")
